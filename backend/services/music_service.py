...
def clean_music_query(text: str) -> str:
    ...
    cleaned = _re.sub(
        r'\b(好的|进行|一下|那个|这个|选首|选个|来帮我|帮我选|选手|选手给|随机|随机多|多放|多|我让|我讓|让你|讓你|随便|隨便|随意|隨意|字幕)\b',
        ' ', cleaned)
    # 9. 合并多余空格
    cleaned = _re.sub(r'\s+', ' ', cleaned).strip()
    # 10. 纯泛化请求 → 返回空串让上游随机播放
    if _re.match(
        r'^(歌曲|歌|音乐|音樂|一首歌|几首歌|来首歌|随便|随便来|随便听|随便听歌|来点音乐|什么歌|啥歌|点儿歌|点歌|听歌|想听歌|'
        r'什么都行|啥都行|都可以|随便什么|什么都可|随意|随机|任意|听音乐|聽音樂|想听|想聽|好的|好|行|可以|行吧|好吧)\s*$',
        cleaned):
        return ""
    # 11. 太短（单字）或太长（>40字）不靠谱
